...
function sourceSection(heading: string, pattern: string[]) {
...
const customization = sourceSection("Customized, Unique Small Appliance Solutions", Array(6).fill(["media", "rich-text", "rich-text"]).flat());
const production = sourceSection("Maximize Product Performance with", Array(8).fill(["media", "rich-text"]).flat());
const proof = sourceSection(heading, Array(4).fill(["media", "rich-text"]).flat());
...
const certification = sourceSection("Globally Recognized Quality Certification", ["rich-text"])[0];
...
export function ProductIndexTemplate({ page }: { page: SitePage }) {
  ...
  return <main id="main-content" className="product-page product-index">
    <header className="product-index__banner"><h1>{cn ? "我们的个护电器为何与众不同？" : heading}</h1></header>
    ...
    <section className="product-index__section product-index__certifications"><h2>{cn ? "全球认可的品质认证" : "Globally Recognized Quality Certification"}</h2><p className="product-index__intro">{cn ? "根据客户要求，产品接受CCC、FCC、CE、IC、RoHS、UL及ISO 9001等技术认证与测试，以满足国际安全、质量和效率标准。" : asText(certification)}</p><SectionRenderer blocks={source.blocks.filter((block) => block.type === "gallery").slice(0, 1)} /></section>
